...
const routeSource = await readFile(new URL('../src/pages/postwire.astro', import.meta.url), 'utf8')
const source = await readFile(new URL('../src/scripts/postwire.ts', import.meta.url), 'utf8')
...
test('Postwire page script is safe to reinitialize across Astro navigation', () => {
  assert.match(routeSource, /<script src="\.\.\/scripts\/postwire\.ts"><\/script>/)
  assert.match(source, /astro:page-load/)
  assert.match(source, /dataset\.initialized/)
  assert.match(source, /setupInstallTabs/)
})

test('Postwire initial render does not start a compositor animation loop', () => {
